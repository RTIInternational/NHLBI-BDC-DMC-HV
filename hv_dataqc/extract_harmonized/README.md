...
- For `MeasurementObservation.tsv` and `Observation.tsv`, detects value columns in this priority order: nested
  ...
  `MeasurementObservation.tsv` also treats `measurement_status` and
  `observation_status` as categorical fallback values. This avoids false
  `n_valid=0` when YAML emits enum/status-valued measurements or uses an
  `object_derivation` for `value_quantity`.
