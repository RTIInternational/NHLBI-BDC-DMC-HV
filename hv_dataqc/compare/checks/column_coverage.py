...
def check_c15_column_coverage(
    ...
    *,
    ...
    return results
    """C15: Entity TSV column schema consistency and required-column presence.

    Emits FAIL when:
    - An entity has different column sets across two or more consent groups.
    - A required column is absent from a loaded entity.
    ...
    for entity in sorted(entity_columns):
        ...
        # ── Sub-check 2: required columns present ───────────────────────────
        if required:
            # Use the union of all consent-group columns as the authoritative
            # set; a column present in ANY group should suppress the warning
            # for that entity.  Per-group gaps are caught by sub-check 1.
            all_seen = set().union(*(set(cols) for _, cols in entries))
            missing_required = [c for c in required if c not in all_seen]
            if missing_required:
                all_ok = False
                # Report the actual columns from the first loaded group.
                results.append(CheckResult(
                    "C15",
                    f"{entity}_required_columns",
                    "FAIL",
                    (
                        f"{entity}.tsv: missing required column(s): "
    ...
    if all_ok:
        ...
        results.append(CheckResult(
            ...
            (
                f"All {len(checked)} loaded entities have consistent column "
                f"schemas across consent groups and required columns present "
                f"({', '.join(checked)})"
            ),
        ))
