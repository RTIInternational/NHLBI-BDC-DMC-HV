...
save "$der\shortdata_$today.dta", replace
...
Units
	...
	- Converstions: create a key spreadsheet of 'this' to 'that' units, with a column of YAML code snippet that will accomplish the conversion
...
/* ----- 1. Clean phv data (FHS) ----- */
use "$temp\\FHS_VariableProperties_right_join_full_drop.dta", clear
drop bdchm_variable
drop study_id dataset_id var_id
foreach var in dbgap_study_accession dataset_accession variable_accession {
	split `var', p(".")
	rename `var'1 x`var'1
	drop `var'*
	}
rename xdbgap_study_accession1 phs
rename xdataset_accession1 pht
rename xvariable_accession1 phv

gen merge_bdchm_label=lower(bdchm_label) 
replace merge_bdchm_label=subinstr(merge_bdchm_label," ","",.)

* Units *;
* temp code:  manually override wrong values inside the extracted FHS file *;
	replace var_units="g/dl" if phv=="phv00007562"
	replace var_units="mg/dl" if inlist(phv,"phv00007564","phv00007559","phv00007558")
* See end of program for future improvements *;
...
duplicates drop
save "$temp\fhs\FHS_clean.dta", replace





/* ----- 2. Merge in documentation (FHS) ----- */
use "$temp\fhs\FHS_clean.dta", clear
...
/* ----- 4. Clean phv data (BDCHM_noFHS_noCOPDGene) ----- */
use "$temp\\Export_BDCHM_noFHS-noCOPDGene_phv_mappings_drop.dta", clear

* Rename & reformat vars *;
drop bdchm_variable variable_accession
rename study_id phs
rename dataset_id pht
rename var_id phv

* Cohort *;
replace cohort=lower(cohort)
replace cohort="hchs_sol" if cohort=="hchs/sol"

* BDCHM variables *;
/*replace source_variable_description=lower(source_variable_description)*/
replace bdchm_label=lower(bdchm_label) 
	replace bdchm_label="stroke" if bdchm_label=="stroke status"
	replace bdchm_label="red blood cell count" if bdchm_label=="red blood cell count volume"
	replace bdchm_label="copd" if bdchm_label=="copd status"
	...
	* Carotid Stenosis *;
	gen left=0
	gen right=0
	replace left=1 if regexm(source_variable_description,"left|lft side")
	replace right=1 if regexm(source_variable_description,"right|rt side")
	replace bdchm_label="carotid stenosis left" if bdchm_label=="carotid stenosis" & left==1
	replace bdchm_label="carotid stenosis right" if bdchm_label=="carotid stenosis" & right==1
	
	* Redact mappings if it's a days since variable *;
	gen time_indic_var=1 if regexm(source_variable_description,"days since|visit year|^age|age at|\(days\)|\(years\)")
	gen oth_indic_var=1 if regexm(source_variable_description,"visit type")
	gen test=1 if regexm(source_variable_description,"\(days\)|\(years\)")
	
	replace bdchm_label="" if time_indic_var==1 & !inlist(bdchm_label,"age at follow-up","death") /*n=399*/
	replace bdchm_label="" if oth_indic_var==1 /*n=40*/
		
gen merge_bdchm_label=subinstr(bdchm_label," ","",.)

* Units *;
gen servday=regexm(source_variable_description,"serv/day|daily|per day")
gen servweek=regexm(source_variable_description,"per week")
gen hrs=regexm(source_variable_description,"how many hours|number of hours|hours")
gen kgm2=regexm(source_variable_description,"kg/m2")
replace var_units="servings/day" if servday==1 & (var_units=="" | var_units=="servings")
replace var_units="servings/week" if servweek==1 & (var_units=="" | var_units=="servings")
replace var_units="hrs" if hrs==1 & var_units==""
replace var_units="kg/m2" if kgm2==1 & var_units==""
...
sort bdchm_label phv enum_code
keep if bdchm_label!="" /*n=439 dropped*/
duplicates drop
drop left right time_indic_var oth_indic_var test serv*
save "$temp\\Export_BDCHM_noFHS-noCOPDGene_phv_mappings_clean.dta", replace





/* ----- 5. Merge in documentation (BDCHM_noFHS_noCOPDGene) ----- */
use "$temp\\Export_BDCHM_noFHS-noCOPDGene_phv_mappings_clean.dta", clear
...
/* ----- 6. Prepare data files as input for code generation (FHS) ----- */
use "$der\alldata_$today.dta", clear
...
* Remove duplicates and unneeded vars *; 
keep bdchm_entity bdchm_label bdchm_varname pht phv onto_id bdchm_unit associatedvisit participantidphv var_units bdchm_unit unit_match has_pht has_onto 
duplicates drop
duplicates list phv /* file must be one row per phv to work due to local macro counting */
save "$der\fhs.dta", replace
...
/* ----- Scan var_desc for unit info ----- */
	* Redact mappings if it's a days since variable *;
	gen time_indic_var=1 if regexm(var_desc,"days since|visit year|^age|age at|\(days\)|\(years\)")
	gen oth_indic_var=1 if regexm(var_desc,"visit type")
	gen test=1 if regexm(var_desc,"\(days\)|\(years\)")
...
* Units *;
gen servday=regexm(var_desc,"serv/day|daily|per day")
gen servweek=regexm(var_desc,"per week")
gen hrs=regexm(var_desc,"how many hours|number of hours|hours")
gen kgm2=regexm(var_desc,"kg/m2")
replace var_units="servings/day" if servday==1 & (var_units=="" | var_units=="servings")
replace var_units="servings/week" if servweek==1 & (var_units=="" | var_units=="servings")
...
/* ----- 6. Prepare data files as input for code generation (BDCHM_noFHS_noCOPDGene) ----- */
...
duplicates list phv 
		/*browse if inlist(phv,"phv00083163")*/	
save "$der\bdchm_nofhscopd.dta", replace
